...
import {
  Activity, Award, ArrowRight, BookOpen, Brain, CalendarDays, ChevronDown, ChevronUp,
  Flame, Gauge, Goal, ListChecks, Medal, Play, Rocket, Settings2, Sparkles,
  Star, Target, Timer, TrendingDown, TrendingUp, Trophy, XCircle, Zap, CheckCircle2,
} from "lucide-react";
import { CountUp } from "@/components/realtime/CountUp";

/* ---------- types matching server payload ---------- */
type Period = { attempts?: number; mcqs?: number; quizzes?: number; mocks?: number; accuracy?: number; studyMinutes?: number };
type Today = Period & { streak?: number; bestStreak?: number; chaptersTouched?: number; customExams?: number };
type Week = Period & { deltaAccuracy?: number; deltaAttempts?: number; bars?: number[] };
type Month = Period & { activeDays?: number };
type Totals = { accuracy: number; avgScore: number; studyMinutes: number; correct: number; wrong: number; mcqs: number; quizzes: number; mocks: number; chaptersCompleted: number; subjectsCovered: number; answered?: number; attempts?: number };
type SeriesPoint = { date: string; minutes: number; accuracy?: number; attempts?: number };
type SubjectAgg = { id: string; name: string; level?: string; color?: string | null; completionPct: number; avgScore: number; weakChapters: number; completedChapters: number; totalChapters: number; attempts: number };
type TimelineItem = { id: string; at: string; kind: string; kindLabel?: string; title?: string; score?: number; subjectName?: string | null };
...
export function HeroBanner({
  name, today, week,
}: { name?: string | null; today?: Today; week?: Week }) {
      ...
      <div className="relative grid grid-cols-1 items-center gap-6 md:grid-cols-[1fr_auto_auto]">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Link to="/mcq-practice" className="inline-flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-xs font-bold text-[oklch(0.4_0.22_280)] shadow-lg transition-transform hover:-translate-y-0.5">
              <Play className="h-3.5 w-3.5" /> Resume practice
            </Link>
            <Link to="/quiz" className="inline-flex items-center gap-1.5 rounded-xl border border-white/30 bg-white/10 px-4 py-2 text-xs font-bold backdrop-blur transition-colors hover:bg-white/20">
        ...
        <div className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur">
          ...
          <p className="font-display mt-2 text-5xl font-bold leading-none"><CountUp value={streak} /></p>
...
function ScoreRing({ value, label, tone, icon: Icon, hint }: { value: number; label: string; tone: string; icon: typeof Gauge; hint: string }) {
    ...
    <div className="glass shadow-card-soft relative overflow-hidden rounded-3xl p-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="font-display mt-3 text-3xl font-bold"><CountUp value={value} />%</p>
          <p className="mt-1 text-[11px] text-muted-foreground">{hint}</p>
        </div>
        <div className="relative h-20 w-20">
          <svg viewBox="0 0 80 80" className="h-full w-full -rotate-90">
            <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="6" fill="none" className="text-muted/30" />
            <circle cx="40" cy="40" r="36" stroke={tone} strokeWidth="6" fill="none" strokeLinecap="round"
              strokeDasharray={`${(value / 100) * C} ${C}`} style={{ transition: "stroke-dasharray 700ms ease" }} />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center font-display text-sm font-bold">{value}</div>
        </div>
      </div>
    </div>
  );
...
export function ScoreRow({ today, week, month, totals }: { today?: Today; week?: Week; month?: Month; totals?: Totals }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <ScoreRing value={consistency} label="Consistency Score" tone={scoreColor(consistency)} icon={Gauge}
        hint={`${month?.activeDays ?? 0} active days · ${today?.streak ?? 0}-day streak`} />
      <ScoreRing value={productivity} label="Productivity Score" tone={scoreColor(productivity)} icon={Zap}
        hint={`${week?.mcqs ?? 0} MCQs · ${fmtMinutes(week?.studyMinutes ?? 0)} this week`} />
      <ScoreRing value={momentum} label="Learning Momentum" tone={scoreColor(momentum)} icon={Rocket}
...
export function GoalTracker({ today, week, month }: { today?: Today; week?: Week; month?: Month }) {
  const items = [
    { label: "Daily goal", icon: Target, current: today?.mcqs ?? 0, target: 20, unit: "MCQs", tone: "var(--neon-purple)" },
    { label: "Weekly goal", icon: CalendarDays, current: week?.mcqs ?? 0, target: 100, unit: "MCQs", tone: "var(--neon-blue)" },
    { label: "Monthly goal", icon: Trophy, current: month?.mcqs ?? 0, target: 400, unit: "MCQs", tone: "var(--neon-pink)" },
  ];
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      {items.map((g) => {
        ...
        return (
          <div key={g.label} className="glass shadow-card-soft rounded-3xl p-5">
            <div className="flex items-center justify-between">
              ...
              {done ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/15 px-2 py-0.5 text-[10px] font-bold text-emerald-500"><CheckCircle2 className="h-3 w-3" /> Done</span>
                : <span className="text-[11px] font-bold" style={{ color: g.tone }}>{pct}%</span>}
            </div>
            <p className="font-display mt-3 text-2xl font-bold"><CountUp value={g.current} /> <span className="text-base text-muted-foreground">/ {g.target} {g.unit}</span></p>
            <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-muted">
              <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${g.tone}, oklch(0.7 0.2 260))`, boxShadow: `0 0 12px ${g.tone}` }} />
            </div>
          </div>
        );
      })}
...
export function SubjectRanking({ subjects }: { subjects: SubjectAgg[] }) {
  const ranked = useMemo(() => {
    ...
    return { strongest: sorted.slice(0, 3), weakest: [...sorted].reverse().slice(0, 3), average: sorted.slice(3, 6) };
  }, [subjects]);
  ...
  return (
    <div className="glass shadow-card-soft rounded-3xl p-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Auto-ranked</span>
      </div>
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        {cols.map((c) => (
          <div key={c.title} className="rounded-2xl bg-background/40 p-4">
            <div className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest" style={{ color: c.tone }}>
              <c.icon className="h-3.5 w-3.5" /> {c.title}
            </div>
            <ul className="mt-3 space-y-2">
              {c.list.length ? c.list.map((s, i) => (
                <li key={s.id} className="flex items-center gap-2 rounded-xl bg-background/50 px-3 py-2 text-xs">
                  <span className="font-display flex h-6 w-6 items-center justify-center rounded-lg text-[11px] font-bold text-white" style={{ background: `linear-gradient(135deg, ${c.tone}, oklch(0.55 0.2 270))` }}>{i + 1}</span>
                  <span className="line-clamp-1 flex-1">{s.name}</span>
                  <span className="font-display font-bold" style={{ color: c.tone }}>{s.avgScore}%</span>
                </li>
              )) : (<li className="rounded-xl border border-dashed border-border p-3 text-center text-[10px] text-muted-foreground">Not enough data yet.</li>)}
            </ul>
          </div>
        ))}
      </div>
 ...
 * =========================================================== */
export function Achievements({ today, totals, month }: { today?: Today; totals?: Totals; month?: Month }) {
  const streak = today?.streak ?? 0;
  ...
  const active = month?.activeDays ?? 0;
  const badges = [
    { label: "7-Day Streak", icon: Flame, unlocked: best >= 7 || streak >= 7, tone: "oklch(0.78 0.16 60)" },
    ...
    { label: "Knowledge Builder", icon: Brain, unlocked: (totals?.chaptersCompleted ?? 0) >= 10, tone: "var(--neon-blue)" },
  ];
  return (
    <div className="glass shadow-card-soft rounded-3xl p-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{badges.filter(b => b.unlocked).length} / {badges.length} unlocked</span>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        {badges.map((b) => (
          <div key={b.label} className={`rounded-2xl border p-3 text-center transition-all ${b.unlocked ? "border-border bg-background/50" : "border-dashed border-border/40 bg-background/20 opacity-50"}`}>
            <span className={`mx-auto flex h-10 w-10 items-center justify-center rounded-xl text-white ${b.unlocked ? "shadow-glow" : ""}`} style={{ background: b.unlocked ? `linear-gradient(135deg, ${b.tone}, oklch(0.55 0.2 270))` : "var(--muted)" }}>
 ...
 * =========================================================== */
export function FocusAnalytics({ timeline, month, productiveDay }: { timeline: TimelineItem[]; month?: Month; productiveDay?: string | null }) {
  const stats = useMemo(() => {
    const hours = new Array(24).fill(0);
    let sessions = 0, totalMin = 0;
    ...
    const hourLabel = sessions ? `${bestHour.toString().padStart(2, "0")}:00 – ${((bestHour + 1) % 24).toString().padStart(2, "0")}:00` : "—";
    return { hourLabel, avgSession, sessions };
  }, [timeline, month]);
  const items = [
    { icon: Timer, label: "Most productive hour", value: stats.hourLabel, tone: "var(--neon-purple)" },
    { icon: CalendarDays, label: "Most productive day", value: productiveDay ?? "—", tone: "var(--neon-blue)" },
    { icon: Activity, label: "Avg session length", value: stats.avgSession ? `${stats.avgSession}m` : "—", tone: "oklch(0.78 0.15 200)" },
  ];
  return (
    <div className="glass shadow-card-soft rounded-3xl p-5">
      <h3 className="font-display flex items-center gap-2 text-lg font-bold">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {items.map((it) => (
          <div key={it.label} className="rounded-2xl bg-background/40 p-4">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl text-white shadow-glow" style={{ background: `linear-gradient(135deg, ${it.tone}, oklch(0.55 0.2 270))` }}>
              <it.icon className="h-4 w-4" />
            </span>
            <p className="mt-2 text-[10px] uppercase tracking-widest text-muted-foreground">{it.label}</p>
...
export function PerformanceComparison({ series }: { series: SeriesPoint[] }) {
  const cmp = useMemo(() => {
    const sum = (arr: SeriesPoint[], pick: (p: SeriesPoint) => number) => arr.reduce((s, p) => s + pick(p), 0);
    const last7 = series.slice(-7), prev7 = series.slice(-14, -7);
    const last30 = series.slice(-30), prev30 = series.slice(-60, -30);
    const mk = (a: SeriesPoint[], b: SeriesPoint[]) => {
      const am = sum(a, (p) => p.minutes), bm = sum(b, (p) => p.minutes);
      ...
      const aSes = sum(a, (p) => p.attempts ?? 0), bSes = sum(b, (p) => p.attempts ?? 0);
      return { minutes: am, minutesDelta: am - bm, accuracy: aAcc, accuracyDelta: aAcc - bAcc, sessions: aSes, sessionsDelta: aSes - bSes };
    };
        ...
        {[
          ...
          return (
            <div key={r.label} className="rounded-2xl bg-background/40 p-3">
              ...
              <p className={`mt-0.5 inline-flex items-center gap-0.5 text-[10px] font-bold ${up ? "text-emerald-500" : "text-rose-500"}`}>
                {up ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />} {up ? "+" : ""}{r.delta}{r.unit}
              </p>
            </div>
          );
        })}
...
export function QuickActions({ wrongCount }: { wrongCount: number }) {
  const items = [
    { to: "/mcq-practice", icon: Play, label: "Resume Last Session", hint: "Continue practicing", tone: "var(--neon-purple)" },
    { to: "/wrong-questions", icon: XCircle, label: "Review Wrong Questions", hint: `${wrongCount} unresolved`, tone: "oklch(0.7 0.2 25)" },
    { to: "/quiz", icon: ListChecks, label: "Continue Study Plan", hint: "Today's tasks", tone: "var(--neon-blue)" },
    { to: "/mock-test", icon: Trophy, label: "Take a Mock Test", hint: "Full syllabus", tone: "var(--neon-pink)" },
  ];
  return (
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      {items.map((it) => (
        <Link key={it.label} to={it.to} className="glass shadow-card-soft group relative overflow-hidden rounded-2xl p-4 transition-transform hover:-translate-y-0.5">
          <div className="pointer-events-none absolute -right-8 -top-8 h-20 w-20 rounded-full opacity-30 blur-2xl transition-opacity group-hover:opacity-60" style={{ background: it.tone }} />
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl text-white shadow-glow" style={{ background: `linear-gradient(135deg, ${it.tone}, oklch(0.55 0.2 270))` }}>
              <it.icon className="h-4 w-4" />
            </span>
  ...
  if (!enabled) {
    return (
      <div className="glass flex items-center justify-between rounded-2xl p-4 text-xs">
        ...
        <button onClick={() => setEnabled(true)} className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-[var(--neon-purple)] to-[var(--neon-blue)] px-3 py-1.5 font-bold text-white">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-border/40 p-5">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">{completed}/{tasks.length} Completed · {pct}%</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="hidden h-2 w-40 overflow-hidden rounded-full bg-muted sm:block">
            <div className="h-full rounded-full bg-gradient-to-r from-[var(--neon-purple)] to-[var(--neon-blue)] transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
          <button onClick={() => setCollapsed((c) => !c)} className="glass rounded-lg p-1.5 text-muted-foreground hover:text-foreground" title={collapsed ? "Expand" : "Collapse"}>
            {collapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}
          </button>
          <button onClick={() => setEnabled(false)} className="glass rounded-lg px-2 py-1 text-[10px] font-bold text-muted-foreground hover:text-foreground" title="Hide plan">Hide</button>
        </div>
      </div>
      {!collapsed && (
        <ul className="divide-y divide-border/40">
          {tasks.map((t) => (
            <li key={t.id} className="flex items-center gap-3 p-4 transition-colors hover:bg-background/40">
              <button onClick={() => toggle(t.id)} aria-label="toggle" className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-all ${t.done ? "border-emerald-500 bg-emerald-500 text-white" : "border-border"}`}>
                {t.done && <CheckCircle2 className="h-3.5 w-3.5" />}
              </button>
              <p className={`flex-1 text-sm ${t.done ? "text-muted-foreground line-through" : "font-semibold"}`}>{t.title}</p>
