...
import {
  Activity, Award, Bell, BookOpen, CheckCircle2, ChevronRight,
  Crown, Edit3, Flame, ListChecks, Loader2, Moon, Sparkles, Sun,
  Target, Timer, TrendingDown, TrendingUp, Trophy, XCircle, Zap, Check,
} from "lucide-react";
import {
...
import {
  getUserGoals, setUserGoals,
  type UserGoals, DEFAULT_USER_GOALS,
...
function goalKey(userId: string) { return `edumaster.goals.v1.${userId}`; }
function readLocalGoals(userId: string): UserGoals {
  ...
  } catch { return { ...DEFAULT_USER_GOALS }; }
}
function writeLocalGoals(userId: string, value: UserGoals) {
  try { window.localStorage.setItem(goalKey(userId), JSON.stringify(value)); } catch { /* ignore */ }
}
...
function motivationalSubtitle(streak: number, accuracy: number, mcqsToday: number) {
  ...
  if (streak >= 7 && accuracy >= 80) return `🔥 ${streak}-day streak with ${accuracy}% accuracy — you're on fire!`;
...
function Ring({ value, size = 120, stroke = 10, gradient = "ring-grad" }: {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        ...
        <motion.circle cx={size / 2} cy={size / 2} r={r} stroke={`url(#${gradient})`}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-display text-2xl font-bold tabular-nums text-foreground">{Math.round(pct)}%</span>
      </div>
    </div>
  );
}

// ───── Top bar: Title + theme toggle + bell + avatar ─────
function PageHeader() {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">Daily Progress</h1>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="glass inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold transition-all hover:scale-[1.03]"
        >
          {theme === "dark"
        ...
        <Link
          ...
          className="glass relative inline-flex h-10 w-10 items-center justify-center rounded-full transition-all hover:scale-[1.05]"
        >
          <Bell className="h-4 w-4" />
          {unread > 0 && (
            <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-pink-500 px-1 text-[10px] font-bold text-white shadow-lg">
              {unread > 9 ? "9+" : unread}
            </span>
          )}
        </Link>
...
// ───── Hero section ─────
function Hero({
  name, streak, overall, motivation,
}: { name: string; streak: number; overall: number; motivation: string }) {
  return (
...
// ───── Stat card ─────
function StatCard({
  icon: Icon, label, value, sub, accent, delta, deltaLabel,
}: {
  icon: typeof ListChecks; label: string; value: React.ReactNode; sub?: string;
  accent: string; delta?: number; deltaLabel?: string;
}) {
  const up = (delta ?? 0) >= 0;
  return (
    <motion.div
    ...
    >
      <div className="pointer-events-none absolute -right-10 -top-10 h-24 w-24 rounded-full opacity-30 blur-3xl transition-opacity group-hover:opacity-60"
        style={{ background: accent }} />
      <div className="relative flex items-start justify-between">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl text-white shadow-md"
          style={{ background: `linear-gradient(135deg, ${accent}, oklch(0.55 0.22 280))` }}>
          <Icon className="h-4 w-4" />
        </span>
      </div>
      <p className="relative mt-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{label}</p>
      ...
      {(sub || delta !== undefined) && (
        <p className="relative mt-1 inline-flex items-center gap-1 text-[11px] text-muted-foreground">
          {delta !== undefined && (
            <span className={up ? "inline-flex items-center text-emerald-500" : "inline-flex items-center text-rose-500"}>
...
// ───── Chart card ─────
function ChartCard({
  title, range, onRange, options, children,
}: {
  title: string; range: RangeKey; onRange: (r: RangeKey) => void;
  options: RangeKey[]; children: React.ReactNode;
}) {
  return (
    <div className="glass shadow-card-soft rounded-3xl p-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        >
          {options.map((o) => <option key={o} value={o}>{RANGE_LABEL[o]}</option>)}
        </select>
      </div>
...
export function DailyProgressCenter() {
  ...
  useEffect(() => { if (goalsQuery.data && user?.id) writeLocalGoals(uid, goalsQuery.data); }, [goalsQuery.data, uid, user?.id]);
  ...
  const trendData = useMemo(() => {
    const n = trendRange === "today" ? 1 : trendRange === "week" ? 7 : trendRange === "month" ? 30 : 30;
  ...
  const wkAttemptsPct = prevWeekAttempts > 0
    ? Math.round((wkAttemptsDelta / prevWeekAttempts) * 100)
    : (wkAttemptsDelta > 0 ? 100 : 0);
  ...
  if ((totals?.mcqs ?? 0) >= 50) achievements.push({
    icon: Trophy, title: "Quiz Master", sub: `Solved ${totals?.mcqs} MCQs`, tone: "from-amber-400 to-rose-500",
  });
  if (accuracyRate >= 70) achievements.push({
    icon: Award, title: "Accuracy Expert", sub: `Maintained ${accuracyRate}% accuracy`, tone: "from-violet-500 to-fuchsia-500",
  });
  if (streak >= 7) achievements.push({
    icon: Flame, title: "Consistency Star", sub: `${streak} days study streak`, tone: "from-orange-400 to-pink-500",
  });
  if ((totals?.mocks ?? 0) >= 5) achievements.push({
    icon: Crown, title: "Mock Champion", sub: `${totals?.mocks} mock tests completed`, tone: "from-cyan-400 to-blue-500",
  });

  const subjBest = subjBreakdown[0];
  const subjWeak = [...subjBreakdown].reverse()[0];
  const subjMostMcqs = [...subjects].sort((a, b) =>
    (b.totalChapters * 10 - a.totalChapters * 10) // proxy
  )[0];
  ...
  return (
    <div className="space-y-6 px-1 pb-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard
          icon={ListChecks} label="MCQs Solved" value={<CountUp value={mcqsSolved} />}
          accent="oklch(0.7 0.27 295)"
          delta={wkAttemptsPct} deltaLabel="vs last week"
        />
        <StatCard
          icon={CheckCircle2} label="Correct Answers" value={correct}
          accent="oklch(0.72 0.18 150)" sub={`${accuracyRate}% Accuracy`}
        />
        <StatCard
          icon={XCircle} label="Wrong Answers" value={wrong}
          accent="oklch(0.7 0.22 25)" sub={`${100 - accuracyRate}% of attempts`}
        />
        <StatCard
          icon={Target} label="Accuracy Rate" value={`${accuracyRate}%`}
          accent="oklch(0.72 0.18 235)" delta={wkAccuracy} deltaLabel="vs last week"
        />
      </div>

      {/* Trend charts */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <ChartCard title="MCQ Solved Trend" range={trendRange} onRange={setTrendRange}
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
              ...
              />
              <Line type="monotone" dataKey="value" stroke="url(#trendG)" strokeWidth={3}
                dot={{ r: 3, fill: "var(--neon-purple)" }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>

        <ChartCard title="Accuracy Over Time" range={accRange} onRange={setAccRange}
          options={["today", "week", "month", "30d"]}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={accData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="value" stroke="oklch(0.65 0.2 150)" strokeWidth={2.5} fill="url(#accG)" />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div className="glass shadow-card-soft rounded-3xl p-5 lg:col-span-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="rounded-md bg-muted/40 px-2 py-0.5 text-[10px] font-medium text-muted-foreground">{RANGE_LABEL.week}</span>
          </div>
          <div className="relative flex h-48 items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={donut} dataKey="value" innerRadius={52} outerRadius={78}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Total MCQs</p>
            </div>
          </div>
        ...
        <div className="glass shadow-card-soft rounded-3xl p-5 lg:col-span-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link to="/qns-bank" className="text-[11px] font-semibold text-[var(--neon-blue)] hover:underline">View All</Link>
          </div>
          {subjBreakdown.length === 0 ? (
            ...
            <div className="space-y-3">
              {subjBreakdown.map((s, i) => {
                const colors = ["var(--neon-purple)", "var(--neon-blue)", "var(--neon-pink)",
                  "oklch(0.72 0.18 150)", "oklch(0.78 0.16 75)"];
                return (
                  <div key={s.id}>
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-muted/40">
                      <motion.div
                        ...
                        style={{ background: `linear-gradient(90deg, ${colors[i % colors.length]}, var(--neon-blue))` }}
                      />
                    </div>
                  </div>
                );
        ...
        <div className="glass shadow-card-soft rounded-3xl p-5 lg:col-span-4">
          ...
          <div className="space-y-2">
            {subjBest && (
              <InsightRow icon={Trophy} title="Best Accuracy" value={`${subjBest.avgScore}%`}
                sub={`On ${subjBest.name}`} tone="from-emerald-400 to-teal-500" />
            )}
            {subjMostMcqs && (
              <InsightRow icon={BookOpen} title="Most MCQs Solved" value={`${subjMostMcqs.completedChapters * 10}`}
                sub={`On ${subjMostMcqs.name}`} tone="from-violet-500 to-blue-500" />
            )}
            {subjWeak && subjWeak.id !== subjBest?.id && (
              <InsightRow icon={Zap} title="Needs Improvement" value={`${subjWeak.avgScore}%`}
                sub={`On ${subjWeak.name}`} tone="from-rose-500 to-pink-500" />
            )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <GoalCard
          ...
          pct={dailyPct}
          variant="ring"
          status={dailyPct >= 100 ? "Daily goal smashed — incredible!" : dailyPct >= 50 ? "Keep it up! You're doing great." : "Let's get started today!"}
        ...
        <GoalCard
          ...
          pct={weeklyPct}
          variant="bar"
          status={weeklyPct >= 100 ? "🎉 Weekly goal complete!" : weeklyPct >= 50 ? "You're halfway there!" : "Plenty of week left — go for it!"}
      ...
      <div className="glass shadow-card-soft rounded-3xl p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link to="/profile" className="text-[11px] font-semibold text-[var(--neon-blue)] hover:underline">View All</Link>
        </div>
        {achievements.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {achievements.slice(0, 4).map((a) => (
              <div key={a.title} className="rounded-2xl border border-border/50 bg-background/30 p-3 text-center transition-transform hover:-translate-y-1">
                <div className={`mx-auto mb-2 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${a.tone} text-white shadow-lg`}>
        ...
        {timeline.length === 0 ? (
          ...
          <div className="space-y-2">
            {timeline.slice(0, 6).map((t) => (
              <div key={t.id} className="flex items-center justify-between rounded-xl border border-border/50 bg-background/30 px-3 py-2.5">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[var(--neon-purple)] to-[var(--neon-blue)] text-white">
                    {t.kind === "mcq_practice" ? <ListChecks className="h-4 w-4" />
                      : t.kind === "quiz" ? <Timer className="h-4 w-4" />
                      : t.kind === "mock" ? <Trophy className="h-4 w-4" />
                      : <Sparkles className="h-4 w-4" />}
                  </span>
                  <div className="min-w-0">
                    <p className="truncate-safe text-xs font-semibold">{t.title}</p>
                    <p className="text-[10px] text-muted-foreground">{t.kindLabel}{t.correct != null ? ` · ${t.correct}/${t.total} correct` : ""}</p>
                  </div>
                </div>
      ...
      <EditGoalDialog
        ...
        onClose={() => { if (!saveGoalsMut.isPending) setEditing(null); }}
        onSave={(value) => {
          if (!editing) return;
          saveGoalsMut.mutate({ [editing]: value }, {
            onSuccess: () => {
...
}: { icon: typeof Trophy; title: string; value: string; sub: string; tone: string }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border/50 bg-background/30 p-2.5">
      <span className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${tone} text-white shadow-md`}>
        <Icon className="h-4 w-4" />
      </span>
...
function GoalCard({
  title, current, target, pct, status, variant, onEdit, flashSaved,
}: {
  title: string; current: number; target: number; pct: number;
  status: string; variant: "ring" | "bar";
  onEdit?: () => void; flashSaved?: boolean;
}) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
    ...
    >
      ...
      {variant === "ring" ? (
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <p className="font-display text-lg font-bold">{current} / {target} MCQs</p>
...
// ───── Edit Goal Dialog ─────
const GOAL_META: Record<"daily" | "weekly" | "monthly", { label: string; hint: string; min: number; max: number; suggest: number }> = {
  daily:   { label: "Daily MCQ Goal",   hint: "How many MCQs do you want to solve each day?",       min: 1, max: 500,   suggest: 20 },
  weekly:  { label: "Weekly MCQ Goal",  hint: "Target MCQs you want to solve over a 7-day window.", min: 1, max: 3000,  suggest: 100 },
  monthly: { label: "Monthly MCQ Goal", hint: "Target MCQs you want to solve over the last 30 days.", min: 1, max: 10000, suggest: 400 },
};

function EditGoalDialog({
  open, kind, current, onClose, onSave, saving, error, fallback,
}: {
  open: boolean;
  ...
  useEffect(() => {
    if (open) { setVal(String(current)); setErr(null); }
  }, [open, current]);
  ...
  const submit = () => {
    const n = Number(val);
    if (!Number.isFinite(n) || !Number.isInteger(n)) { setErr("Enter a whole number."); return; }
    if (n < meta.min) { setErr(`Minimum is ${meta.min}.`); return; }
    if (n > meta.max) { setErr(`Maximum is ${meta.max}.`); return; }
    onSave(n);
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3 py-2">
          <div className="space-y-1.5">
            <Label htmlFor="goal-input" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">MCQs</Label>
            <Input
              id="goal-input"
              type="number"
              inputMode="numeric"
            ...
            {fallback && (
              <p className="text-[11px] text-amber-500">
                Saved locally — run the <code className="rounded bg-muted px-1">user_goals</code> SQL setup to sync across devices.
              </p>
            )}
          </div>
          <div className="flex flex-wrap gap-2">
            {[meta.suggest, Math.round(meta.suggest * 1.5), meta.suggest * 2].map((s) => (
              <button
                key={s}
                type="button"
                onClick={() => { setVal(String(s)); setErr(null); }}
