...
import {
  ArrowLeft, Search, Download, Loader2, BadgeCheck, Crown,
  ShieldCheck, Activity, Filter as FilterIcon,
} from "lucide-react";
...
export const Route = createFileRoute("/admin/users/list")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Filtered drilldown of users across status, role, verification and activity." },
    ],
  }),
});
...
function AdminUsersListPage() {
  ...
  const sorted = useMemo(() => {
    const arr = [...rows];
    switch (search.sort) {
      case "name": arr.sort((a, b) => a.display_name.localeCompare(b.display_name)); break;
      case "logins": arr.sort((a, b) => (b.total_login_count ?? 0) - (a.total_login_count ?? 0)); break;
      case "usage": arr.sort((a, b) => (b.total_usage_seconds ?? 0) - (a.total_usage_seconds ?? 0)); break;
      case "lastLogin": arr.sort((a, b) => new Date(b.last_login_at ?? 0).getTime() - new Date(a.last_login_at ?? 0).getTime()); break;
  ...
  function exportCsv() {
    const header = ["id", "name", "email", "level", "status", "verified", "roles", "logins", "usage_seconds", "last_login_at", "created_at"];
    const lines = [header.join(",")].concat(
      sorted.map((r) => [
        r.id, csv(r.display_name), csv(r.email ?? ""), csv(r.level), csv(r.status),
        r.email_verified ? "yes" : "no", csv((r.roles ?? []).join("|")),
        r.total_login_count ?? 0, r.total_usage_seconds ?? 0,
    ...
    <div className="space-y-5 p-4 lg:p-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {isFetching ? "Loading…" : `${total.toLocaleString()} matching ${total === 1 ? "user" : "users"}`}
            </p>
          </div>
      ...
      <section className="glass shadow-card-soft grid grid-cols-1 gap-3 rounded-2xl p-4 md:grid-cols-5">
        <div className="relative md:col-span-2">
          ...
          <Input
            ...
            onBlur={() => { if (q !== search.q) update({ q }); }}
            placeholder="Search name, email, or user id…"
            className="rounded-xl pl-9"
          />
        </div>
        <Select value={search.status ?? "all"} onValueChange={(v) => update({ status: v === "all" ? undefined : (v as z.infer<typeof statusEnum>) })}>
        ...
        <Select value={search.role ?? "all"} onValueChange={(v) => update({ role: v === "all" ? undefined : (v as z.infer<typeof roleEnum>) })}>
        ...
        <Select value={search.sort} onValueChange={(v) => update({ sort: v as z.infer<typeof sortEnum> })}>
      ...
      <section className="glass shadow-card-soft overflow-hidden rounded-2xl">
            ...
            <tbody>
              ...
              {sorted.map((u) => (
                <tr key={u.id} className="border-t border-border/40 hover:bg-muted/20">
                  ...
                  <td className="px-3 py-3 text-muted-foreground">{fmtDateTime(u.last_login_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border/40 px-3 py-2 text-xs text-muted-foreground">
          <div>Page {search.page} of {totalPages}</div>
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="rounded-lg" disabled={search.page <= 1} onClick={() => update({ page: search.page - 1 })}>Prev</Button>
            <Button size="sm" variant="outline" className="rounded-lg" disabled={search.page >= totalPages} onClick={() => update({ page: search.page + 1 })}>Next</Button>
          </div>
        </div>
      </section>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${map[status] ?? "bg-muted text-muted-foreground"}`}>{status}</span>;
}
